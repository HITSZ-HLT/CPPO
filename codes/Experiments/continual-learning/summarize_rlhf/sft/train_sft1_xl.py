...
import numpy as np
import torch
from summarize_dataset import TLDRDataset_CL
...
if __name__ == "__main__":
    ...
    MODEL_PATH = f"trlx/examples/summarize_rlhf/download/{MODEL_VERSION}"
    output_dir = f"checkpoint_CL/{MODEL_VERSION}_sft{datapart}" #checkpoint_CL/gpt2-s_sft1
    ...
    train_dataset = TLDRDataset_CL(
        data_path,
        tokenizer,
        "train",
        max_length=max_input_length,
        datapart=f"Data{datapart}"
    )
    dev_dataset = TLDRDataset_CL(
        data_path,
        tokenizer,
        "valid",
        max_length=max_input_length,
