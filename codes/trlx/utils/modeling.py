...
def logprobs_of_labels_mask(logits, labels, topP=0.2):
    """Log probabilities of the labels
    ...
    if topP == 1.0:
        ...
        # qt = torch.quantile(logits.detach().cpu().float(), torch.tensor(1-topP), -1).to(logits.device)
        # logits[logits<qt.unsqueeze(dim=-1)] = -100.0

        N = logits.shape[-1]
        v, _ = logits.detach().cpu().float().topk(k=int((1 - topP) * N), largest=False, dim=-1)
        qt = v.max(dim=-1)[0].to(logits.device)
        logits[logits <= qt.unsqueeze(dim=-1)] = -5.0
