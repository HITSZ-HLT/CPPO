...
class HPPOOrchestrator(Orchestrator):
    """Dynamic PPO Orchestrator
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            for sample_idx in range(n_samples):
                ...
                ppo_rl_PandR.append(torch.tensor([all_logprobs[sample_idx].mean() ,rewards[-1]]))
            ...
            #
            old_rewards = torch.tensor([item.rewards[-1] for item in ppo_rl_elements[-n_samples:]]).to(device)
        ...
        stats["time/make_exp"] = time()-time_make_exp


        coef_stats = self.trainer.push_to_store(ppo_rl_elements)
        stats.update(coef_stats)
